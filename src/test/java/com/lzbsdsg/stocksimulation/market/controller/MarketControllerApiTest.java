...
class MarketControllerApiTest {
  ...
  @MockBean private MarketApplicationService marketApplicationService;
  ...
  void should_get_realtime_metrics() throws Exception {
    when(marketApplicationService.getRealtimeMetrics())
        .thenReturn(
            new MarketRealtimeMetricsVO(
                ...
                false,
                2.0d,
                new MarketLatencyMetricVO(
                    "market.ingest.cycle.duration", 10L, 8.2d, 15.6d, 12.0d, 14.5d),
                new MarketLatencyMetricVO(
                    "market.pubsub.fanout.delay", 10L, 3.1d, 7.0d, 5.6d, 6.5d),
                new MarketLatencyMetricVO("market.ws.queue.delay", 50L, 6.4d, 20.0d, 12.3d, 18.2d),
                new MarketLatencyMetricVO(
                    "ws_push_duration_seconds", 50L, 1.1d, 5.4d, 3.2d, 4.8d)));
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.data.ingestCycleLatency.metric").value("market.ingest.cycle.duration"));
  }
}
