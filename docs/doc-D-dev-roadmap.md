...
> 目标：一人全栈，20 周完成 MVP → Beta 发布（高并发架构）
...
- [x] 行情中心新增大盘数据（含成交量/成交额）与涨跌幅榜展示。
- [x] 行情中心改为全市场分页展示（单页 30/40 只），并支持切页。
- [x] 实时订阅范围升级为“自选股 + 当前页股票 + 热点集合”并集，切页仅替换当前页来源集合。
- [x] 大盘/榜单改为后端代理官方源（`/market/indexes`、`/market/rank-board`），前端不再直连官方源。
- [x] 新增 `GET /api/v1/market/listed` 供前端加载股票池并执行本地分页（不承担榜单排序职责）。
- [x] 行情抓取与查询链路升级为多源并发融合（新浪/腾讯并发，超时受控）。
- [x] 新增融合策略：按新鲜度 + 完整度 + 有效变化判定缓存覆盖与推送。
- [x] 缓存写入升级为条件覆盖（防止旧数据晚到覆盖新数据）。
- [x] Pub/Sub 与 WS 推送仅在行情有效变化时触发，减少页面闪烁。
- [x] 榜单与大盘降级链路升级为：官方源 -> 多源融合 -> 最近成功缓存（后端全程兜底）。
...
- 行情中心可展示大盘信息（价格/涨跌/成交量/成交额）与涨跌幅榜。
- 行情中心分页可用（30/40 每页），翻页后保持并集实时订阅且仅当前页来源发生切换。
- 官方榜单源波动时，后端仍能稳定返回融合榜单或缓存榜单，前端不出现明显闪烁。
...
**任务**：
- [x] 编写开发 Dockerfile（后端多阶段 + 前端 Nginx）
- [x] 编写 docker-compose.dev.yml（高并发架构版）:
  ```yaml
  services:
...
- [x] 配置 application.yml（主从数据源、Redis Cluster 节点列表）
- [x] 健康检查: `/actuator/health` 含 DB/Redis/MQ 状态

**验收标准**：
- `docker compose -f docker-compose.dev.yml up -d` 一键启动全部服务（2 App + PG主从 + Redis Cluster + MQ + Nginx）
...
- [ ] OWASP ZAP 快速扫描（可选）
- [ ] 发布前按需收敛 Swagger UI（非开发环境）
- [ ] 发布前按需收敛 Actuator 暴露端点
- [ ] CORS 白名单收紧
